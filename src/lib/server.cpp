...
class chat_server {
public:
    chat_server(boost::asio::io_context& io_context, const tcp::endpoint& endpoint, std::string cid) 
...
public:
    net::awaitable<void> Acceptor() try {
        while (true) {
            ...
            try {
                ...
                connection_pool_->AddConnection(reply.client_cid, CreateConnection(io_context_, std::move(socket), cid_));
...
Server::Server(boost::asio::io_context& io_context, std::string cid) 
    : io_context_(io_context)
      , endpoint_(tcp::v4(), 0) 
      , own_cid_(cid)
{
    impl_ = std::make_unique<chat_server>(io_context_, endpoint_, cid);
...
void Server::MakeConnectionTo(std::string ip, uint16_t port, std::string cid) {
    impl_->connection_pool_->AddConnection(cid, CreateConnection(ip, port, io_context_, own_cid_));
}
